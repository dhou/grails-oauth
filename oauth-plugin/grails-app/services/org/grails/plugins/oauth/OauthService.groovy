...
class OauthService implements InitializingBean {
    ...
    void reset() {
        ...
        // init HttpClient
        if(httpClient) {
            httpClient.getConnectionManager().shutdown(); // release old connections on service reset
        }
        
        HttpParams clientParams = new BasicHttpParams()
        HttpProtocolParams.setVersion(clientParams, HttpVersion.HTTP_1_1)
        HttpProtocolParams.setContentCharset(clientParams, HTTP.UTF_8)
        HttpProtocolParams.setUseExpectContinue(clientParams, false) // twitter requires this off

        SchemeRegistry schemeRegistry = new SchemeRegistry()
        schemeRegistry.register(new Scheme("http", PlainSocketFactory.getSocketFactory(), 80))
        schemeRegistry.register(new Scheme("https", SSLSocketFactory.getSocketFactory(), 443))

        ClientConnectionManager manager = new ThreadSafeClientConnManager(clientParams, schemeRegistry)
     ...
     *         if {@code method} is neither unset (defaults to 'GET') nor one of 'GET', 'PUT', 'POST', 'DELETE', 'HEAD', or 'OPTIONS'.
     * @throws OauthServiceException
     *         if {@code token} does not contain both a 'key' and 'secret'
     * @throws OauthServiceException
     *         if {@code consumer} does not represent an existing consumer
     * @throws IOException
    ...
    */
    def accessResource(final def url, final def consumer, final def token,
        ...
        accessResource(url: url, consumer: consumer, token: token, method: method, params: params, headers: headers, body: body, accept: accept, contentType: contentType)
    }
     ...
     *        Valid arguments: url, consumer, token, method, params, headers, body, accept, and contentType.<br/>
     * @return the response body as a {@code String}.
     * @throws UnsupportedOperationException
     *         if {@code args.method} is neither unset (defaults to 'GET') or one of 'GET', 'PUT', 'POST', 'DELETE', 'HEAD', or 'OPTIONS'.
     * @throws OauthServiceException
    ...
    def accessResource(final def Map args) {
        HttpResponse response = doRequest(args)
        String body = ""
        try {
            ...
            String message = "Error reading response body"
    ...
    */
    def doRequest(final def url, final String consumer, final def token,
        ...
        doRequest(url: url, consumer: consumer, token: token, method: method, params: params, headers: headers, body: body, accept: accept, contentType: contentType)
    }
    ...
    */
    def doRequest(final Map args) {
        ...
        if (!token || !token?.key || !token?.secret) {
            final def errorMessage = "Unable to access protected resource, invalid token: " +
                ...
                "headers = $headers, contentType: $contentType, body: ${body?.length() < 100 ? body : body?.substring(0, 99) + '...'}"
        ...
        final HttpUriRequest requestMethod;
        ...
        if(parameters.size() > 0) {
            if(!body && (!contentType || contentType?.equalsIgnoreCase("x-www-form-urlencoded")) && method == "POST") { // POSTs with params, no body, and no contentType should use x-www-form-urlencoded
                ...
                def queryString = "?" + URLEncodedUtils.format(parameters, HTTP.UTF_8)
                log.debug("Adding params as query string: ${queryString}")
        ...
        execute(requestMethod, consumer, token) // sign, execute and return HttpResponse
    }
     ...
     * @throws OAuthCommunicationException
     */
    def HttpResponse execute(final HttpUriRequest request, final String consumerName, final def accessToken) {
        assertAccessTokenFormat(accessToken)
        log.debug("Executing ${request?.getMethod()} to ${request?.getURI()}")

        sign(request, consumerName, accessToken) // sign the request
        httpClient.execute(request) // execute it, return the HttpResponse
    }
     ...
     * @throws OAuthCommunicationException
     */
    def sign(HttpUriRequest request, final String consumerName, final def accessToken) {
        assertAccessTokenFormat(accessToken)
    ...
    */
    private def getProvider(final String providerName) {
        ...
        if(!providerValues) {
            throw new OauthServiceException("Unknown provider: '$providerName'")
        }
        return new CommonsHttpOAuthProvider(providerValues.requestTokenUrl, providerValues.accessTokenUrl, providerValues.authUrl, httpClient)
    }

    /**
     * Ensures that an access token has both a 'key' and 'secret'
